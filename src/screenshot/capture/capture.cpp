...
#ifdef EH_HAVE_LIBPNG
static bool write_png16_from_linear(const float* rgb, int w, int h, const char* path) {
...
#ifdef EH_HAVE_LIBJXL
static bool write_jxl_hdr(const float* rgb, int w, int h, const char* path, uint32_t max_lum) {
...
std::vector<OutputInfo> list_outputs(hs::core::WaylandConnection& wl)
{
  ...
  for (const auto& b : bounds) {
    ...
    if (color_mgr) {
      auto ci = hs::core::query_output_color_info(display, color_mgr, b.output);
      oi.is_hdr = ci.is_hdr;
    ...
    HdrData* out_hdr)
{
  ...
  if (color_mgr && out_hdr) {
    ...
    if (want_hdr) {
      ...
      if (!batch_ok && wlr_mgr) {
        batch_ok = hs::core::batch_capture_outputs(display, wlr_mgr, shm, batch, false, color_mgr, wl.linux_dmabuf());
      }
    ...
    HdrData* out_hdr)
{
  ...
  auto fill_batch = [&]() {
    ...
    for (const auto& o : outputs) {
      hs::core::BatchedCaptureOutput bco{};
      bco.output = o.output;
  ...
  auto composite_batch = [&](std::vector<hs::core::BatchedCaptureOutput>& batch) -> bool {
